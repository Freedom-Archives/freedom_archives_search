export const up = async function (knex) {
  ...
  await knex.raw(`
      ...
      DROP VIEW IF EXISTS unified_records;
...
ALTER TABLE _unified_collections ALTER COLUMN child_records TYPE jsonb;

ALTER TABLE _unified_collections ALTER COLUMN featured_records TYPE jsonb;

ALTER TABLE _unified_collections ALTER COLUMN parent TYPE jsonb;

ALTER TABLE _unified_collections ALTER COLUMN children TYPE jsonb[];

ALTER TABLE _unified_records ALTER COLUMN media TYPE jsonb;

ALTER TABLE _unified_records ALTER COLUMN collection TYPE jsonb;

ALTER TABLE _unified_records ALTER COLUMN children TYPE jsonb[];

ALTER TABLE _unified_records ALTER COLUMN siblings TYPE jsonb[];

ALTER TABLE _unified_records ALTER COLUMN parent TYPE jsonb;

ALTER TABLE _unified_records ALTER COLUMN continuations TYPE jsonb[];

ALTER TABLE collections_snapshots ALTER COLUMN children TYPE jsonb;
 ...
 SELECT b.collection_id,
    ...
    to_jsonb(array_agg(to_jsonb(( SELECT i.*::record AS i
           FROM ( SELECT a.list_item_id,
                    a.item) i)) ORDER BY a.item)) AS items,
...
CREATE OR REPLACE VIEW unified_collections AS
 SELECT a.collection_id,
    ...
    to_jsonb(ARRAY( SELECT jsonb_build_object('record_id', b_1.record_id, 'title', b_1.title, 'parent_record_id', b_1.parent_record_id, 'primary_media_thumbnail', primary_media.thumbnail, 'primary_media_format_id', primary_media.format_id, 'primary_media_format_text', list_items.item, 'record_type', frt.record_type) AS jsonb_build_object
           FROM records b_1
...
CREATE OR REPLACE VIEW unified_records AS
 SELECT a.record_id,
   ...
   FROM records a
     ...
     LEFT JOIN LATERAL ( SELECT COALESCE(array_agg(to_jsonb(cr.*) ORDER BY (array_position(c_1.continuation_records, cr.record_id))), '{}'::jsonb[]) AS continuations
